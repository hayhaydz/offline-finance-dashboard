...
export async function wipeUserData(db: DB, userId: number): Promise<void> {
	...
	if (goals.length) {
		await db
			.delete(schema.goalAllocations)
			.where(inArray(schema.goalAllocations.goalId, goals.map((g) => g.id)));
	}
	...
	if (accounts.length) {
		await db
			.delete(schema.accountBalances)
			.where(inArray(schema.accountBalances.accountId, accounts.map((a) => a.id)));
	}
