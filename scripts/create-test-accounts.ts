...
async function main() {
	...
	// Create balances
	const balance1 = await db.insert(schema.accountBalances).values({
	...
	const balance2 = await db.insert(schema.accountBalances).values({
		accountId: account1[0].id,
	...
	console.log('✅ Test data created. Run npm run migrate:slugs to generate slugs.');
