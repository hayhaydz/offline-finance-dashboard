...
async function main() {
  ...
  const userId = existingAdmin ? existingAdmin.id : (await db.query.users.findFirst({
    where: eq(schema.users.username, 'admin')
  }))!.id;
  ...
  // 9. Create goals with Monzo-style pots (explicit allocations)
  ...
  if (existingGoals.length > 0) {
    console.log(`\n⚠️  ${existingGoals.length} goals already exist. Skipping goal creation.`);
  } else {
    console.log('\n🎯 Creating savings goals...');
    const goalsToCreate = [
    {
      name: 'Emergency Fund',
      targetAmountInCents: 4000000, // £40,000 (12 months expenses)
      isEmergencyFund: true,
      targetDate: null,
      allocations: [
        { accountId: 3, amount: 150000 }, // From Emergency Fund savings account
        { accountId: 1, amount: 50000 },   // From Main Current
      ]
    },
    {
      name: 'House Deposit',
      targetAmountInCents: 5000000, // £50,000
      isEmergencyFund: false,
      targetDate: new Date('2027-12-31'),
      allocations: [
        { accountId: 4, amount: 80000 },   // From ISA Saver
        { accountId: 5, amount: 100000 },  // From Stocks & Shares ISA
      ]
    },
    {
      name: 'Holiday Fund',
      targetAmountInCents: 300000, // £3,000
      isEmergencyFund: false,
      targetDate: new Date('2026-08-01'),
      allocations: [
        { accountId: 1, amount: 20000 },   // From Main Current
      ]
    }
  ];

  const createdGoals = [];
  for (let index = 0; index < goalsToCreate.length; index++) {
    const goalData = goalsToCreate[index];
    const goalSlug = nanoid(16);
    const now = new Date();
    const created = await db.insert(schema.goals).values({
      slug: goalSlug,
      userId,
      name: goalData.name,
      targetAmountInCents: goalData.targetAmountInCents,
      currentAllocation: 0, // Will be updated by allocations
      targetDate: goalData.targetDate,
      isEmergencyFund: goalData.isEmergencyFund,
      sortOrder: index, // Set sort order based on array position
      deletedAt: null,
      createdAt: now,
      updatedAt: now
    }).returning();
    createdGoals.push({ goal: created[0], allocations: goalData.allocations });
    console.log(`  ✓ Created goal: ${goalData.name} (sort_order: ${index})`);
  }

  // 10. Create goal_allocations ledger entries
  console.log('\n📝 Creating goal allocation ledger entries...');

  for (const { goal, allocations } of createdGoals) {
    ...
    for (const allocation of allocations) {
      ...
      await db.insert(schema.goalAllocations).values({
        ...
        amount: allocation.amount, // Positive for USER_ADD
        type: 'USER_ADD',
        allocationDate,
        createdAt: allocationDate
      });
    ...
    await db.update(schema.goals)
      .set({ currentAllocation: totalAllocated })
