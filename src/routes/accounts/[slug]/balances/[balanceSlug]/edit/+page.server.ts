...
export const load: PageServerLoad = async ({ locals, params }) => {
	...
	devLog("editBalance", "Loading edit balance page", {
		accountSlug,
		balanceSlug,
	});
	...
	if (!account) {
		logError("editBalance", "Account not found", {
			accountSlug,
	...
	if (account.closedAt) {
		logError("editBalance", "Attempt to edit balance on closed account", {
			accountSlug,
		});
...
export const actions: Actions = {
	...
	updateBalanceEntry: async ({ request, locals, params }) => {
		...
		try {
			...
			return fail(400, {
				error: "Invalid balance format. Enter amount like 123.45 or 123",
			});
		}
