...
function getDebtStatusClass(ttz: {
	months: number | null;
	years: number | null;
}): string {
...
export const load: PageServerLoad = async ({ locals }) => {
	...
	const withMetrics = await Promise.all(
		liabilities.map(async (account) => {
			...
			const progress = account.originalPrincipal
				? (
						((account.originalPrincipal - balance) /
	...
	const revolving = withMetrics.filter((a) => a.type === "credit-card");

	const installment = withMetrics.filter(
		(a) => a.type === "loan" || a.type === "mortgage",
	);

	// Calculate summary totals
	const totalDebt = withMetrics.reduce((sum, a) => sum + a.balance, 0);
	const totalMonthlyInterest = withMetrics.reduce(
		(sum, a) => sum + (a.projection[0]?.interest || 0),
		0,
	);
