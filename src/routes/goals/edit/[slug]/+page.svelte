<script lang="ts">
	...
	let { data, form } = $props<{
		data: { goal: Goal; user: { id: number; username: string; createdAt: Date } };
		form: any;
	}>();

	// Form field values initialized to empty, will sync from data
	let name = $state('');
	...
	// Sync all values from goal data on mount and from form data on failed submission
	$effect(() => {
		if (form?.data) {
			...
			// Initialize from goal data
			name = data.goal.name;
			targetAmount = formatCurrency(data.goal.targetAmountInCents);
			goalType = data.goal.goalType;
			targetDate = data.goal.targetDate ? new Date(data.goal.targetDate).toISOString().split('T')[0] : '';
		}
	});

	// Initialize checkbox states from goal data
	$effect(() => {
		if (!form?.data) {
			try {
				const parsedAccountTypes = JSON.parse(data.goal.accountTypeFilters) as string[];
				const parsedLiquidity = JSON.parse(data.goal.liquidityFilters) as string[];
...
<div class="border-b border-black p-2">
	<h1 class="text-lg font-bold mb-2 mt-0">EDIT GOAL</h1>
	<p class="text-gray-600 my-1">Update your savings goal settings</p>
</div>
...
>
	...
	<input type="hidden" name="slug" value={data.goal.slug} />
