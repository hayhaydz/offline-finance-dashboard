<script lang="ts">
	...
	const accountTypes = $derived.by(() => {
		const typeMap = new Map<string, { count: number; excluded: boolean; category: 'asset' | 'liability' }>();
	...
	const assetTypes = $derived(
		Array.from(accountTypes.entries()).filter(([_, data]) => data.category === 'asset')
	);

	const liabilityTypes = $derived(
		Array.from(accountTypes.entries()).filter(([_, data]) => data.category === 'liability')
	);
	...
	>
		...
		>
			...
			<div class="p-2">
				...
				<!-- Footer note -->
				<div class="text-xs text-gray-600 mt-2 pt-2 border-t border-dotted border-gray-400">
					Excluded accounts are ignored in net worth but still shown in Accounts section.
				</div>
			</div>

			<!-- Footer with save/cancel -->
			<div class="flex justify-between px-2 py-2 bg-gray-100 border-t border-black">
				<form action="?/updateExclusions" method="POST" onsubmit={() => {
					console.log('[ExclusionsModal] Submitting checkbox states:', Object.fromEntries(checkboxStates.entries()));
				}} use:enhance={() => {
					return async ({ result, update }) => {
						if (result.type === 'success') {
							console.log('[ExclusionsModal] Form success, updating page data');
							await update();
							console.log('[ExclusionsModal] Page data updated, closing modal');
							handleFormSuccess();
						}
					};
				}}>
					...
					{#each Array.from(checkboxStates.entries()) as [type, excluded]}
						<input type="hidden" name="type_{type}" value={excluded ? '1' : '0'} />
					{/each}
