...
<div class="max-w-3xl mx-auto font-mono text-xs uppercase relative pb-32">
  ...
  <div class="space-y-8 px-4">
    ...
    <div class="w-full">
      <div class="flex justify-between mb-1 font-bold {colors.text}">
        <span>07 :: BLOCK_CHAIN</span>
        <span>[{Math.floor(p/5)} BLOCKS]</span>
      </div>
      <div class="h-6 w-full bg-gray-100 relative border border-transparent">
        <div class="absolute inset-0 pattern-blocks opacity-20"></div>
        <div class="h-full overflow-hidden transition-all duration-300 relative" style="width: {p}%">
          <div class="h-full w-screen pattern-blocks {colors.text}"></div>
        </div>
      </div>
    </div>
...
<style>
  ...
  /* V7: Blocks */
  .pattern-blocks {
    background-image: repeating-linear-gradient(90deg, transparent, transparent 2px, currentColor 2px, currentColor 10px, transparent 10px, transparent 12px);
  }
  /* V8: Dither */
  .pattern-hash {
